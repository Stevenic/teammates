...
// ── color.ts ────────────────────────────────────────────────────────
import { color, colorBlend, colorBrighten, colorShade, TRANSPARENT, BLACK, WHITE, RED, GREEN, BLUE, YELLOW, CYAN, MAGENTA, GRAY, DARK_GRAY, LIGHT_GRAY, } from "../pixel/color.js";
// ── box-pattern.ts ──────────────────────────────────────────────────
import { UP, RIGHT, DOWN, LEFT, BOX_NONE, BOX_CHARS, boxChar, mergeBoxPatterns, } from "../pixel/box-pattern.js";
// ── symbol.ts ───────────────────────────────────────────────────────
import { sym, charWidth, EMPTY_SYMBOL } from "../pixel/symbol.js";
// ── foreground.ts ───────────────────────────────────────────────────
import { foreground, blendForeground, EMPTY_FOREGROUND, } from "../pixel/foreground.js";
// ── background.ts ───────────────────────────────────────────────────
import { background, blendBackground, EMPTY_BACKGROUND, } from "../pixel/background.js";
// ── pixel.ts ────────────────────────────────────────────────────────
import { pixel, blendPixel, PIXEL_EMPTY, PIXEL_SPACE, } from "../pixel/pixel.js";
// ── buffer.ts ───────────────────────────────────────────────────────
...
describe("foreground", () => {
    ...
    describe("blendForeground()", () => {
        ...
        it("merges style flags via OR when both have box patterns", () => {
            const below = foreground(sym(boxChar(LEFT | RIGHT), LEFT | RIGHT), RED, { bold: true });
            const above = foreground(sym(boxChar(UP | DOWN), UP | DOWN), BLUE, { italic: true });
            const result = blendForeground(above, below);
