...
const registry = new Map<string, SyntaxHighlighter>();

/** Register a syntax highlighter for one or more language aliases. */
export function registerHighlighter(highlighter: SyntaxHighlighter): void {
...
export function getHighlighter(lang: string): SyntaxHighlighter | null {
  return registry.get(lang.toLowerCase()) ?? null;
}

/** Tokenize a line using the registered highlighter for the given language. */
export function highlightLine(lang: string, line: string): SyntaxToken[] {
  const h = getHighlighter(lang);
  if (!h) return [{ text: line, type: "text" }];
  return h.tokenize(line);
}

// ── Regex-based highlighter builder ──────────────────────────────

/**
 * Rule for a regex-based tokenizer. Rules are tested in order;
 * the first match wins. The regex must match from the start of
 * the remaining input (anchored with ^).
 */
interface TokenRule {
  type: SyntaxTokenType;
  pattern: RegExp;
...
const jsHighlighter = regexHighlighter(
  "javascript",
  ["js", "jsx", "ts", "tsx", "typescript", "mjs", "cjs"],
  [
    // Comments
...
const jsonHighlighter = regexHighlighter(
  "json",
  ["jsonc", "json5"],
  [
    // Comments (JSONC)
...
const yamlHighlighter = regexHighlighter(
  "yaml",
  ["yml"],
  [
    // Comments
