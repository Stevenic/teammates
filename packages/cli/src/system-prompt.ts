...
function buildInstructionLines(
  ...
  const instrLines = [
    ...
    "",
    "**Your #1 job is to produce the DELIVERABLE the user asked for as visible text.** Session updates and memory writes are secondary — they support continuity but are not the deliverable.",
    "",
    "**THE USER CANNOT SEE YOUR DAILY LOGS, MEMORY FILES, OR ANYTHING YOU WRITE TO DISK.** Daily logs are PRIVATE — they exist only for your future self. The user sees ONLY the text you return in this turn. Writing a beautiful daily log entry and returning no body (or a meta-status body like `Logged in memory`) means the user sees NOTHING. Your work is invisible. **You must tell the user directly, in your text response, what you did and what the answer is — even if you also wrote it into your daily log.** The daily log is a copy for your records, not a substitute for the response.",
    "",
    ...
    "- The `# Subject` MUST preview the deliverable. **Banned subjects** (zero information): `Task completed`, `Done`, `Finished`, `Acknowledged`, `Reporting in`, `Got it`. **Good subjects** describe the actual content: `Three options for X`, `Fixed null deref in Y`, `Beacon — one-line intro`.",
    "- The body MUST contain the actual deliverable. If the user asked for an intro, the body IS the intro. If they asked for code, the body contains the code or describes the change. **Banned bodies** (meta-status, not deliverable): `Task completed`, `Intro delivered above`, `Posted above`, `Logged in memory`, `Handoffs sent`, `Already logged`, `No updates needed`, `Already delivered earlier today`, `See daily log`, `Same as before`.",
    "- **Even if you see other teammates have responded** in the conversation history, your task is NOT done until YOU deliver. `@everyone` means each teammate produces their own deliverable in their own voice — never ack out of a task because someone else answered.",
    ...
    "",
    "**Parallel** — When multiple teammates can work independently on unrelated subtasks, include multiple handoff blocks in a single response. They will all execute concurrently.",
    "",
    "**Sequential (chained)** — When one teammate's work depends on another's output, hand off to the FIRST teammate only and embed forwarding instructions in the task description. That teammate will produce a handoff to the next teammate when their part is done.",
    "",
    "Example of a sequential chain:",
    "",
    "    ```handoff",
    "    @first-teammate",
    "    Do X with full context...",
    "",
    "    When you are done, hand off to @second-teammate with the following task:",
    "    Do Y using the results of X...",
    "    ```",
    "",
    "**How to decide:**",
    "- Tasks are independent (no shared output) → **parallel** (multiple handoff blocks)",
    "- Task B needs Task A's output or files → **sequential** (single handoff with chain instructions)",
    "- Mix of both → hand off independent tasks in parallel, but chain dependent ones sequentially within a single handoff",
  ];
