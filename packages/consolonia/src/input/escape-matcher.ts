...
export class EscapeMatcher implements IMatcher {
  ...
  private handleCsi(char: string, code: number): MatchResult {
  ...
  private handleSs3(char: string): MatchResult {
    this.state = State.Idle;
  ...
  private parseModifier(): { shift: boolean; ctrl: boolean; alt: boolean } {
    if (!this.params) {
