...
- **Folder naming convention.** Every child folder of `.teammates/` is classified by its name prefix: no prefix = teammate (e.g., `beacon/`), `_` prefix = shared non-teammate folder that gets checked in (e.g., `_standups/`, `_tasks/`), `.` prefix = local/ephemeral folder that is gitignored (e.g., `.tmp/`). The CLI detects teammates by looking for unprefixed folders containing a `SOUL.md`.
...
- **Memory search is built in.** `@teammates/recall` is bundled with the CLI and automatically queries the vector index before every task. Relevant episodic summaries and typed memories are injected into each teammate's context — no manual search needed.
